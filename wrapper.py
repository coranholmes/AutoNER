    ...
    def nvidia_memory_map(use_logger=True, gpu_index=None):
        ...
        if use_logger:
            ...
            if not gpu_index:
                ...
                logger.info("GPU {}: {} (Used Memory)\t {} (Utils)".format(gpu_index, gpu_memory_map[gpu_index][0],
                                                                           gpu_memory_map[gpu_index][1]))
...
class wrapper(basic_wrapper):
    ...
    def __init__(self,
                 ...
                 name = None,
                 seed = None,
                 enable_git_track = False,
                 sheet_track_name = None,
    ...
    def auto_device(self, metrics='memory', use_logger=True, required_minimal=None, wait_time=20):
        ...
        return basic_wrapper.auto_device(metrics=metrics, use_logger=use_logger, required_minimal=required_minimal,
                                         wait_time=wait_time)
    ...
    def add_model_update_stats(self,
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                ...
                self.writer.add_scalar("parameter_gradient_update/" + name, update_norm / (param_norm + 1e-7),
                                       batch_index)
    ...
    def add_model_parameter_histograms(self,
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                self.writer.add_histogram("parameter_histogram/" + name, param.clone().detach().cpu().data.numpy(),
                                          batch_index)
